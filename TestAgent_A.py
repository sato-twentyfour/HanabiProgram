...
class TestAgent_A(Player):
    ...
    def choice_act(self):
        ...
        if act_num == 1:##play playable card
            ...
            return 'd' + player[0].seeing_board.discarded_card.color + str(player[0].seeing_board.discarded_card.number)
            ...
            player[0].seeing_board.played_card = player[self.player_index].pick_card(self.act_index)
            player[0].seeing_board.play_processing()
    ...
    def can_take_riskyact(self):
        ...
        for i in range(len(color_list) - 1):
            for j in range(HANDNUM):
                if max_PO < playable_PO_value[i][j]:
                    max_PO = playable_PO_value[i][j]
                    ##max_PO_clr_index = i
                    max_PO_h_index = j

        if max_PO >= self.risk_aversion:
            self.act_index = max_PO_h_index
            act_num = 6
            print("take risk action")
